...
file_SWC  <- "/Users/ecor/ownCloud/CoE/SWC_h_B2_calibrated.csv" #/Volumes/AlpEnv/Projekte/HiResAlp/06_Workspace/BrJ/02_data/Station_data_Mazia/processed/SWC/SWC_h_B2.csv"
...
datadir <- "/Users/ecor/Dropbox/R-packages/geotopOptim/data_new"
...
file_SWC <- list.files(datadir,pattern="SWC_h_B2_calibrated.csv",full.name=TRUE)[1]
...
tz <- "Etc/GMT+1"
SWC$Date <- as.POSIXct(as.character(SWC$Date),format=format_date,tz=tz)
SWP$Date <- as.POSIXct(as.character(SWP$Date),format=format_date,tz=tz)
SoilTemp$Date <- as.POSIXct(as.character(SoilTemp$Date),format=format_date,tz=tz)
...
datapath <- datadir ##"/Users/ecor/ownCloud/activity/R-Packages/geotopOptim/data"
